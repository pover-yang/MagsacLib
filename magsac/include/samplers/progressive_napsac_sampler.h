...
namespace sampler {
    ...
    protected:
        std::unique_ptr<utils::UniformRandomGenerator> random_generator; // The random number generator
    ...
    bool ProgressiveNapsacSampler<_DimensionNumber>::initialize(const cv::Mat *container_) {
        ...
        for (size_t layer_idx = 0; layer_idx < layer_number; ++layer_idx) {
            if (layer_idx > 0 &&
                ...
                fprintf(stderr,
                        "Error when initializing the Progressive NAPSAC sampler. The layers must be in descending order. The current order is \"");
    ...
    OLGA_INLINE bool ProgressiveNapsacSampler<_DimensionNumber>::sample(
        ...
        if (sample_size_ != sample_size) {
            fprintf(stderr,
                    "An error occured when sampling. Progressive NAPSAC is not yet implemented to change the sample size after being initialized.\n");
            return false;
        }
