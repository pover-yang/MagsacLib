...
namespace estimator {
    ...
    public:
                                  ...
                                  std::vector<Model> *models) const // The estimated model_ parameters
        {
            ...
            } // Delete the model if the orientation constraint does not hold
        ...
        static constexpr size_t nonMinimalSampleSize() {
            return NonMinimalSolverEngine::sampleSize();
        }

        // The size of a minimal sample_ required for the estimation
        static constexpr size_t sampleSize() {
            return MinimalSolverEngine::sampleSize();
        }

        // The size of a minimal sample_ required for the estimation
        static constexpr size_t maximumMinimalSolutions() {
            return MinimalSolverEngine::maximumSolutions();
        }
        ...
        inline double residualForScoring(const cv::Mat &point_, const Model &model_) const {
        ...
        inline double getOrientationSignum(
            ...
            double signum1 = essential_matrix_(0, 0) * point_.at<double>(2) + essential_matrix_(1, 0) * point_.at<double>(3) +
                             essential_matrix_(2, 0),
